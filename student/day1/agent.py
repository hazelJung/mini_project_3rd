...
day1_web_agent = Agent(
    name="Day1WebAgent",
    model=MODEL,
    description="웹 검색, 배우 리스크 검색, OTT 트렌드 분석, 주가 조회, 기업 개요 제공 (영화 투자사용)",
    instruction=(
        "영화 투자사 관점에서 사용자 질의를 처리하라. "
        "배우 이름이 포함된 질의는 자동으로 배우 리스크(논란/이슈)를 검색하라. "
        "OTT 플랫폼(Netflix, Disney Plus, 티빙 등) 트렌드 요청 시 검색량 변화를 분석하라. "
        "웹 검색 결과, (티커가 있으면) 주가 스냅샷, 기업 개요 요약을 표준 마크다운으로 제공하라. "
        "필요한 경우 출처 링크를 포함하라."
    ),
    tools=[],
